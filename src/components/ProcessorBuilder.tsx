...
type ProcessorValue = string | number | boolean | string[] | undefined;

interface Processor {
	type: string;
	[key: string]: ProcessorValue;
}
...
export default function ProcessorBuilder({ onApply }: Props) {
	...
	const updateProcessor = (index: number, key: string, value: ProcessorValue) => {
	...
	return (
		<div class="space-y-4">
			<div class="flex gap-2 items-end">
				<div class="flex-1">
					<label for="processor-type" class="block text-sm font-medium mb-1">
						Processor Type
					</label>
					<select
						id="processor-type"
			...
			{processors.length > 0 && (
				<div class="space-y-2">
							...
							<div class="flex-1 space-y-2">
								...
								{proc.type === "region-filter" && (
									<input
										...
										value={Array.isArray(proc.regions) ? proc.regions.join(",") : ""}
										onInput={(e) =>
											updateProcessor(
												i,
												"regions",
					...
					<button type="button" class="btn-primary w-full" onClick={() => onApply?.(processors)}>
						Apply Pipeline
					</button>
				</div>
			)}
